...
def make_secure_token(*args, **options):
    ...
    if hasattr(token_value, 'encode'):  # pragma: no cover
        token_value = token_value.encode('utf-8')  # ensure bytes

    print (token_value)
    return token_value

def convert_dictkeys(d):
    return { key.decode(): val for key, val in d.items() }
    ...
    def oauth2callback(self, view_func):
        ...
        def decorated(*args, **kwargs):
            ...
            if 'state' in request.args:
                params.update(**convert_dictkeys(self.parse_state(request.args.get('state'))))
